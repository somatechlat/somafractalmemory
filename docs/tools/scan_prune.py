...
def write_outputs(inv):
    # write CSV
    keys = ['path','size','sha256','is_binary','ref_count','referenced_by','duplicate_group','risk_score','recommended_action']
    with OUT_CSV.open('w', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        w.writerow(keys)
        for it in inv:
            w.writerow([it.get(k) if k not in ('referenced_by','duplicate_group') else (";".join(it.get(k)) if it.get(k) else '') for k in keys])
    # write JSON
    simple = []
    for it in inv:
    ...
    with OUT_MD.open('w', encoding='utf-8') as f:
        ...
        for it in top_candidates:
            ...
            f.write(f"|{it['path']}|{it['size']}|{it['risk_score']}|{it['recommended_action']}|{it['ref_count']}|{dup}|\n")
        f.write('\n\n')
        f.write('## Notes\n')
        f.write('- `DELETE_CANDIDATE` means low references and high risk score; still requires manual review.\n')

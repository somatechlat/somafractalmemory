...
def test_cli_path(tmp_path, capsys):
    ...
    for idx, coord in enumerate(("1,1,1", "2,2,2"), start=1):
        cfg_path.write_text(json.dumps({"redis": {"testing": True}, "qdrant": {"path": str(tmp_path / f"qp_{idx}.db")}}))
        cli.sys = __import__('sys')
        cli.sys.argv = [
            "soma", "--mode", "local_agent", "--namespace", "cli_path", "--config-json", str(cfg_path),
    ...
    # Link
    cfg_path.write_text(json.dumps({"redis": {"testing": True}, "qdrant": {"path": str(tmp_path / "qp_link.db")}}))
    cli.sys.argv = [
        "soma", "--mode", "local_agent", "--namespace", "cli_path", "--config-json", str(cfg_path),
        "link", "--from", "1,1,1", "--to", "2,2,2", "--type", "related"
    ]
    cli.main()
    _ = capsys.readouterr()

    # Path
    cfg_path.write_text(json.dumps({"redis": {"testing": True}, "qdrant": {"path": str(tmp_path / "qp_path.db")}}))
    cli.sys.argv = [
        "soma", "--mode", "local_agent", "--namespace", "cli_path", "--config-json", str(cfg_path),
        "path", "--from", "1,1,1", "--to", "2,2,2"
    ]
    cli.main()

...
def main() -> None:
    ...
    parser.add_argument('--config-json', dest='config_json', default=None, help='Path to JSON config file')
    ...
    p_store.add_argument('--type', dest='mem_type', default='episodic', choices=['episodic', 'semantic'])
    ...
    p_del_many.add_argument('--coords', required=True, help='Semicolon-separated coords, e.g., "1,2,3;4,5,6"')

    p_store_bulk = sub.add_parser('store-bulk', help='Store memories from JSON file')
    p_store_bulk.add_argument('--file', required=True, help='Path to JSON array of items {coord, payload, type}')

    p_range = sub.add_parser('range', help='Find memories within a coordinate bounding box')
    p_range.add_argument('--min', dest='min_coord', required=True, help='Comma-separated min coord, e.g., 0,0,0')

...
store_latency = Histogram(
    "soma_memory_store_latency_seconds", "Store operation latency", ["namespace"]
)
recall_count = Counter("soma_memory_recall_total", "Total recall operations", ["namespace"])
recall_latency = Histogram(
    "soma_memory_recall_latency_seconds", "Recall operation latency", ["namespace"]
)
...
def _point_id(namespace: str, coord: tuple[float, ...]) -> str:
    ...
    try:
        ...
        return "pt-" + hashlib.blake2b(f"{namespace}:{repr(coord)}".encode()).hexdigest()
...
class MemoryManager:
    ...
    def embed_text(self, text: str) -> np.ndarray:
        ...
        try:
            self.kv_store.set(
                cache_key, pickle.dumps(vec.tolist(), protocol=pickle.HIGHEST_PROTOCOL)
            )
        except Exception:
    ...
    def store(self, coordinate: tuple[float, ...], value: dict):
        ...
        try:
            ...
            try:
                ...
                store_latency.labels(namespace=self.namespace).observe(
    ...
    def get_all_memories(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(
                f"Vector store unavailable for get_all_memories: {e}, falling back to key-value store."
            )
    ...
    def find_hybrid_by_type(
        ...
        if memory_type:
            filtered = [
                (p, d)
                for p, d in zip(payloads, distances, strict=False)
                if p.get("memory_type") == memory_type.value
            ]
            payloads, distances = zip(*filtered, strict=False) if filtered else ([], [])
        if filters:
            ...
            filtered = [(p, d) for p, d in zip(payloads, distances, strict=False) if ok(p)]
            payloads, distances = zip(*filtered, strict=False) if filtered else ([], [])

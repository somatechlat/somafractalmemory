...
try:
    ...
    # Module-level metric handles (initialized lazily). Keep them None-safe so that
    # the runtime works even if prometheus_client is not installed or metrics are
    # already registered elsewhere (tests may re-import or create registries).
    _STORE_COUNT = None
    _STORE_LATENCY = None
    _RECALL_COUNT = None
    _RECALL_LATENCY = None
    _EVICTION_PRUNED = None
    _VECTOR_UPSERT_TOTAL = None
    _PREDICTION_REQUESTS_TOTAL = None
    _EVICTION_INDEX_SIZE = None
    _WAL_PENDING = None

    def _init_metrics():
        """Initialize module-level Prometheus metrics in the default registry.

        This function is safe to call multiple times; duplicate registration
        attempts are caught and ignored so tests/imports remain stable.
        """
        global _STORE_COUNT, _STORE_LATENCY, _RECALL_COUNT, _RECALL_LATENCY
        global _EVICTION_PRUNED, _VECTOR_UPSERT_TOTAL, _PREDICTION_REQUESTS_TOTAL
        global _EVICTION_INDEX_SIZE, _WAL_PENDING
        ...
        try:
            # Counters
            _STORE_COUNT = Counter(
                "soma_memory_store_total",
                "Total store operations",
                ["namespace"],
                registry=PROM_REGISTRY,
            )
            _RECALL_COUNT = Counter(
                "soma_memory_recall_total",
                "Total recall operations",
                ["namespace"],
                registry=PROM_REGISTRY,
            )
            _EVICTION_PRUNED = Counter(
                "soma_eviction_pruned_total",
                "Total number of evicted/pruned memories",
                [],
                registry=PROM_REGISTRY,
            )
            _VECTOR_UPSERT_TOTAL = Counter(
                "soma_vector_upsert_total",
                "Total vector upsert operations",
                ["namespace"],
                registry=PROM_REGISTRY,
            )
            _PREDICTION_REQUESTS_TOTAL = Counter(
                "soma_prediction_requests_total",
                "Total prediction provider requests",
                ["namespace"],
                registry=PROM_REGISTRY,
            )

            # Histograms
            _STORE_LATENCY = Histogram(
            ...
            # Gauges
            _EVICTION_INDEX_SIZE = Gauge(
                "soma_eviction_index_size",
                "Current size of the eviction index (zset)",
                ["namespace"],
                registry=PROM_REGISTRY,
            )
            _WAL_PENDING = Gauge(
                "soma_wal_entries_pending",
                "Pending WAL entries waiting reconciliation",
                ["namespace"],
                registry=PROM_REGISTRY,
            )
        except Exception:
            # If any metric fails to register (e.g. duplicate name), leave handles as None
            try:
                # Best-effort: if PROM_REGISTRY supports querying, try to bind existing
                # metrics by name. Keep this non-fatal.
                pass
            except Exception:
                pass
...
# Ensure metric globals exist regardless of import outcome (prometheus_client may or
# may not be available). This avoids NameError when the import succeeded and the
# except-branch definitions were not executed.
if "_STORE_COUNT" not in globals():
    _STORE_COUNT = None
if "_STORE_LATENCY" not in globals():
    _STORE_LATENCY = None
if "_RECALL_COUNT" not in globals():
    _RECALL_COUNT = None
if "_RECALL_LATENCY" not in globals():
    _RECALL_LATENCY = None
if "_EVICTION_PRUNED" not in globals():
    _EVICTION_PRUNED = None
if "_VECTOR_UPSERT_TOTAL" not in globals():
    _VECTOR_UPSERT_TOTAL = None
if "_PREDICTION_REQUESTS_TOTAL" not in globals():
    _PREDICTION_REQUESTS_TOTAL = None
if "_EVICTION_INDEX_SIZE" not in globals():
    _EVICTION_INDEX_SIZE = None
if "_WAL_PENDING" not in globals():
    _WAL_PENDING = None

if "_init_metrics" not in globals():

    def _init_metrics():
        # No-op placeholder if prometheus client is not available; real
        # implementation may have been defined above during import-time.
        return
...
class SomaFractalMemoryEnterprise:
    ...
    def _reconcile_once(self):
        ...
        # Update WAL pending gauge if available
        try:
            if self._m_wal_pending is not None:
                # best-effort count of pending WAL entries
                pending = sum(1 for _ in self.kv_store.scan_iter(f"{wal_prefix}*"))
                try:
                    self._m_wal_pending.set(pending)
                except Exception:
                    pass
        except Exception:
            pass
    ...
    def __init__(
        ...
        try:
            # Respect explicit pin via SOMA_MODEL_REV when provided
            if hf_rev:
                ...
                # Only allow unpinned downloads if explicitly opted-in via SOMA_ALLOW_UNPINNED_HF
                # to avoid unexpected network downloads in secure environments.
                allow_unpinned = os.getenv("SOMA_ALLOW_UNPINNED_HF", "false").lower() == "true"
                if allow_unpinned:
                    # Intentional unpinned download (opt-in).  # nosec B615
                    self.tokenizer = AutoTokenizer.from_pretrained(hf_model)
                    self.model = AutoModel.from_pretrained(hf_model, use_safetensors=True)
                else:
                    # Keep tokenizer/model as None to force fallback embeddings.
                    self.tokenizer = None
                    self.model = None
        except (OSError, RuntimeError, ValueError) as e:
            # These are expected errors from missing network, missing PyTorch, or bad model names.
            logger.warning(
        ...
        # Initialize or bind to shared Prometheus metrics. Use module-level singletons
        # so multiple instances do not attempt duplicate registration on the default
        # registry. Metrics are intentionally low-cardinality (only `namespace` label).
        try:
            _init_metrics()
        except Exception:
            # If prometheus client not present or initialization fails, fall back to
            # attributes that are None-safe in call sites.
            pass

        # Instance-level metric handles (bound to this namespace)
        self._m_store_count = _STORE_COUNT.labels(self.namespace) if _STORE_COUNT else None
        self._m_store_latency = _STORE_LATENCY.labels(self.namespace) if _STORE_LATENCY else None
        self._m_recall_count = _RECALL_COUNT.labels(self.namespace) if _RECALL_COUNT else None
        self._m_recall_latency = _RECALL_LATENCY.labels(self.namespace) if _RECALL_LATENCY else None
        self._m_eviction_pruned = _EVICTION_PRUNED
        self._m_vector_upsert_total = (
            _VECTOR_UPSERT_TOTAL.labels(self.namespace) if _VECTOR_UPSERT_TOTAL else None
        )
        self._m_prediction_requests = (
            _PREDICTION_REQUESTS_TOTAL.labels(self.namespace)
            if _PREDICTION_REQUESTS_TOTAL
            else None
        )
        self._m_eviction_index_size = (
            _EVICTION_INDEX_SIZE.labels(self.namespace) if _EVICTION_INDEX_SIZE else None
        )
        self._m_wal_pending = _WAL_PENDING.labels(self.namespace) if _WAL_PENDING else None
    ...
    def _serialize(self, obj: Any) -> bytes:
        ...
        try:
            ...
            if os.getenv("SOMA_ALLOW_PICKLE", "false").lower() == "true":
                # Allow pickle only when explicitly enabled via SOMA_ALLOW_PICKLE.
                # This is an intentional opt-in for trusted environments.  # nosec B403
    ...
    def _deserialize(self, raw: Any) -> Any:
        ...
        if isinstance(raw, bytes):
            try:
                ...
                if os.getenv("SOMA_ALLOW_PICKLE", "false").lower() == "true":
                    # Pickle deserialization is gated behind SOMA_ALLOW_PICKLE. Only enable in
                    # trusted environments.  # nosec B403
                    import pickle as _pickle
    ...
    def store(self, coordinate: Tuple[float, ...], value: dict):
        ...
        try:
            ...
            # Vector upsert metric
            try:
                if self._m_vector_upsert_total is not None:
                    self._m_vector_upsert_total.inc()
            except Exception:
                pass
            ...
            try:
                ...
                try:
                    ...
                    # increment WAL pending gauge after creating an entry
                    try:
                        if self._m_wal_pending is not None:
                            # best-effort pending count
                            pending = sum(
                                1 for _ in self.kv_store.scan_iter(f"{self.namespace}:wal:*")
                            )
                            self._m_wal_pending.set(pending)
                    except Exception:
                        pass
        ...
        try:
            ...
            # Observe store latency and increment counters
            try:
                dur = time.time() - start
                if self._m_store_latency is not None:
                    self._m_store_latency.observe(dur)
                if self._m_store_count is not None:
                    self._m_store_count.inc()
            except Exception:
                pass
                ...
                try:
                    ...
                    if not raw_data:
                        ...
                        try:
                            ...
                            # Best-effort coord parsing failed; ignore and continue. Log for debug.
                            logger.debug(f"Coord fallback parse failed for {meta_key}: {e}")
                    if not raw_data:
                        continue
                    ...
                    try:
                        ...
                        # Logger f-string here is not an SQL expression; Bandit false-positive B608.  # nosec B608
                        logger.debug(
    ...
    def retrieve(self, coordinate: Tuple[float, ...]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if lock:
                with lock:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Failed to update eviction index on access for {data_key}: {e}"
                        )
            ...
            try:
                dur = time.time() - r_start
                if self._m_recall_latency is not None:
                    self._m_recall_latency.observe(dur)
                if self._m_recall_count is not None:
                    self._m_recall_count.inc()
            except Exception:
                pass
    ...
    def _enforce_memory_limit(self):
        try:
            ...
            if excess <= 0:
                try:
                    if self._m_eviction_index_size is not None:
                        self._m_eviction_index_size.set(int(total))
                except Exception:
                    pass
                return
            ...
            for member in to_remove:
                try:
                    ...
                    self.kv_store.delete(data_key)
                    self.kv_store.delete(meta_key)
                    # increment eviction pruned counter
                    try:
                        if self._m_eviction_pruned is not None:
                            self._m_eviction_pruned.inc()
                    except Exception:
                        pass
                except Exception as e:
            ...
            try:
                ...
                try:
                    if self._m_eviction_index_size is not None:
                        # update gauge after eviction
                        new_size = max(0, int(total) - excess)
                        self._m_eviction_index_size.set(new_size)
                except Exception:
                    pass
            except Exception:
                pass
